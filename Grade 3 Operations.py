...
class WorksheetGenerator:
    ...
    def generate_3digit_addition_no_regrouping(self, count: int) -> pd.DataFrame:
        ...
        while len(problems) < count:
            h1 = random.randint(1, 4); t1 = random.randint(0, 4); o1 = random.randint(0, 4)
            h2 = random.randint(1, 5 - h1); t2 = random.randint(0, 5 - t1); o2 = random.randint(0, 5 - o1)
            op1 = h1 * 100 + t1 * 10 + o1
            op2 = h2 * 100 + t2 * 10 + o2
            if (o1 + o2 < 10) and (t1 + t2 < 10) and (op1 + op2 < 1000):
                problems.append({'operand1': op1, 'operand2': op2, 'result': op1 + op2})
        return pd.DataFrame(problems)

    def generate_3digit_subtraction_no_regrouping(self, count: int) -> pd.DataFrame:
        """Generate 3-digit subtraction problems without regrouping"""
        problems = []
        while len(problems) < count:
            h1 = random.randint(2, 9); t1 = random.randint(1, 9); o1 = random.randint(1, 9)
            h2 = random.randint(1, h1 - 1); t2 = random.randint(0, t1); o2 = random.randint(0, o1)
            op1 = h1 * 100 + t1 * 10 + o1
            op2 = h2 * 100 + t2 * 10 + o2
    ...
    def generate_3digit_addition_with_regrouping(self, count: int) -> pd.DataFrame:
        """Generate 3-digit addition problems with regrouping"""
        problems = []
        while len(problems) < count:
            h1 = random.randint(1, 7); t1 = random.randint(1, 9); o1 = random.randint(1, 9)
            h2 = random.randint(1, 8 - h1); t2 = random.randint(0, 9); o2 = random.randint(0, 9)
            op1 = h1 * 100 + t1 * 10 + o1
            op2 = h2 * 100 + t2 * 10 + o2
            if (o1 + o2 >= 10) and (op1 + op2 < 1000):
                problems.append({'operand1': op1, 'operand2': op2, 'result': op1 + op2})
        return pd.DataFrame(problems)

    def generate_3digit_subtraction_single_borrowing(self, count: int) -> pd.DataFrame:
        """Generate 3-digit subtraction problems with single borrowing"""
        problems = []
        while len(problems) < count:
            h1 = random.randint(2, 9); t1 = random.randint(1, 9); o1 = random.randint(0, 8)
            h2 = random.randint(1, h1 - 1); t2 = random.randint(0, t1); o2 = random.randint(o1 + 1, 9)
            op1 = h1 * 100 + t1 * 10 + o1
            op2 = h2 * 100 + t2 * 10 + o2
            if (o1 < o2) and (t1 >= t2) and (op1 > op2):
                problems.append({'operand1': op1, 'operand2': op2, 'result': op1 - op2})
    ...
    def generate_3digit_subtraction_double_borrowing(self, count: int) -> pd.DataFrame:
        """Generate 3-digit subtraction problems with double borrowing"""
        problems = []
        while len(problems) < count:
            h1 = random.randint(3, 9); t1 = random.randint(0, 8); o1 = random.randint(0, 8)
            h2 = random.randint(1, h1 - 1); t2 = random.randint(t1 + 1, 9); o2 = random.randint(o1 + 1, 9)
            op1 = h1 * 100 + t1 * 10 + o1
            op2 = h2 * 100 + t2 * 10 + o2
            if (o1 < o2) and (t1 < t2) and (op1 > op2):
                problems.append({'operand1': op1, 'operand2': op2, 'result': op1 - op2})
    ...
    def generate_division_tables(self, count: int) -> pd.DataFrame:
        """Generate exact-division problems using times tables up to 10"""
        problems = []
        while len(problems) < count:
            op2 = random.randint(2, 10)          # divisor
            mult = random.randint(2, 10)          # multiplier
            op1 = op2 * mult                      # dividend (exact)
            problems.append({'operand1': op1, 'operand2': op2, 'result': op1 // op2})
        return pd.DataFrame(problems)
    ...
    def generate_word_problems(self) -> List[dict]:
        ...
        for _ in range(2):
            while True:
                op1 = random.randint(100, 700)
                op2 = random.randint(100, 999 - op1)
                result = op1 + op2
                if 1 <= result <= 999:
        ...
        for _ in range(2):
            while True:
                op1 = random.randint(200, 999)
                op2 = random.randint(100, op1 - 1)
                result = op1 - op2
                if 1 <= result <= 999:
        ...
        for _ in range(2):
            while True:
                op1 = random.randint(2, 10)
                op2 = random.randint(2, min(10, 999 // op1))
                result = op1 * op2
                if 1 <= result <= 999:
    ...
    def generate_latex(self) -> str:
        ...
        seq_by_100 = self.generate_sequence_by_100()

        add_3digit_regroup   = self.generate_3digit_addition_with_regrouping(3)
        sub_3digit_double    = self.generate_3digit_subtraction_double_borrowing(3)
        add_3digit_noregroup = self.generate_3digit_addition_no_regrouping(1)
        sub_3digit_single    = self.generate_3digit_subtraction_single_borrowing(3)
        sub_3digit_nogroup   = self.generate_3digit_subtraction_no_regrouping(2)
        mult = self.generate_multiplication_tables(12)
        div  = self.generate_division_tables(12)
        word_problems = self.generate_word_problems()

        # Mixed addition/subtraction pool
        mixed_pool = (
            pd.concat([add_3digit_regroup, sub_3digit_double, add_3digit_noregroup, sub_3digit_single, sub_3digit_nogroup])
            .sample(frac=1)
            .head(12)
...
\title{\vspace{-2cm} Grade 3 -- Monthly Assessment - 2}
\date{\vspace{-1cm} February, 2026}

\pagestyle{headandfoot}
\firstpagefooter{''' + self.student_name + r'''}{Grade 3 - Revision Worksheet, February, 2026}{\thepage}
\runningfooter{''' + self.student_name + r'''}{Grade 3 - Revision Worksheet, February, 2026}{\thepage}
\firstpageheader{}{}{}
\runningheader{}{}{}
...
'''
        ...
        latex += self._sequence_rows_block(seq_by_100)
        ...
        # ---------- Q5: Division ----------
        latex += r'\newpage' + '\n'
        latex += r'\question Solve the following:' + '\n\n'
        div_label = 0
        for start, end in [(0, 5), (5, 10)]:
            ...
            for i in range(start, end):
                row = div.iloc[i]
                op1 = int(row['operand1'])
                op2 = int(row['operand2'])
                lbl = chr(ord('a') + div_label)
                div_label += 1
                latex += (
                    r'(' + lbl + r') $' + str(op1) + r' \div ' + str(op2)
                    + r' = \underline{\hspace{0.8cm}}$' + '\n\n'
                )
            latex += r'\end{multicols}' + '\n'
            if start == 0:
...
def generate_all_worksheets(
    ...
    for student_name in student_list:
        ...
        output_file = os.path.join(output_dir, f"grade3_assessment2_{safe_name}.tex")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Grade 3 Monthly Assessment 2 — Worksheet Generator")
    print("=" * 60)
